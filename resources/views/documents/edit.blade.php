...
<div class="breadcrumb-header justify-content-between special ltr ltr-elem">
    <div class="my-auto">
        <div class="d-flex">
            ...
            <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ modifier {{ $document->name }}</span>
        </div>
    </div>
</div>
<!-- breadcrumb -->
    ...
    <div class="col-xl-12">
        <div class="card mg-b-20">
            ...
            <div class="card-header pb-0 ltr ltr-elem">
                ...
                <p class="tx-12 tx-gray-500 mb-2">Ici, vous pouvez editer les information du votre document souhaitee </p>
            </div>

            <div class="container p-5">

                <form action="/documents/update" method="post" autocomplete="off">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="row mt-4" style="flex-direction: row-reverse;">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" name="name" id="name" value="{{ $document->name }}" class="form-control ltr" placeholder=" veuillez enter le nom du document" autofocus>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <label for="section" class="ltr-elem">Section du document</label>
                            <select id="section" name="section" class="form-control select1 ltr">
                                <option class="select-c" label="Veuillez choisir une section">
                                </option>

                                @foreach ($sections as $section)
                                <option value="{{ $section->id }}" {{ ($section->id == ($document->section ? $document->section->id : null)) ? 'selected' : '' }}>
                                    {{ $section->name }}
                                </option>
                                @endforeach
                    ...
                    <div class="col-mt-4">
                        <label for="client" class="ltr-elem">Nom du client </label>
                        <select id="client" name="client" class="form-control select2 ltr" style="text-align: left">
                            ...
                            <option value="{{ $client->id }}" {{ ($document->client->id  == $client->id) ? "selected" : "" }}>
                                {{ $client->name }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="row mt-4">
                        <div class="col-md-12">
                            ...
                            <textarea id="description" name="description" class="form-control ltr " placeholder="Veuillez saisir une petite description sur le document" rows="4">{{ $document->description }}</textarea>
                        </div>
                    </div>


                    <button type="submit" class="btn btn-outline-primary mt-5" style="display: block;margin: 0 auto;">
                        Confirmer &nbsp; &nbsp;<i class="fa fa-plus"></i>
                    </button>
